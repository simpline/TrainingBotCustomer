...
module.exports = (robot) ->
  ...
  robot.respond /MovableTypeに変更したことを報告しに来ました。/i, (res) ->
    res.reply "いやー、変えてもらったところを申し訳ないんだけどね。やはりWordpressのがよさそうで、Wordpressにしてほしいんだよ。MovableTypeがいいと言っていた社員もブログを書くだけでサーバを運用するわけじゃないから、口出さないように言っておいたよ。それでは、よろしく。"
    res.send "「@boss お客様からWordpressに戻して100万PVに耐えられるようにしてほしいと頼まれました。」の形式です。"

  robot.respond /Wordpressに戻したことを報告しに来ました。/i, (res) ->
    res.reply "じゃあ、早速使ってみますよ。それと、せっかく書いたのに消えてしまってはいやだから、バックアップをとれるようにしておいてください。"
